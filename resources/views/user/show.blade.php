        ...
        <main role="main">
            <div class="row justify-content-center">
                <div class="col-xl-10 col-xxl-8">
                    ...
                    <div class="card mb-3">
                        <div class="card-header" data-bs-toggle="collapse" href="#userDataContent" role="button" aria-expanded="true" aria-controls="userDataContent">
                            ...
                            <div class="card-body">
                                ...
                                <div class="">
                                    @can('user-update', $user)
                                        <a href="{{ route('users.edit', $user->id) }}" data-bs-toggle="tooltip" title="Editar usuário" class="btn btn-primary btn-sm">
                    ...
                    @component('employee.componentBasicEmployeeData', [
                        'employee'=>$user->employee, 
                        'no_employee_message' => 'O usuário não possui colaborador associado.'
                    ])@endcomponent

                    <div class="card mb-3">
                        <div class="card-header" data-bs-toggle="collapse" href="#utaListContent" role="button" aria-expanded="true" aria-controls="utaListContent">
                            <h4 class='mb-0'>Papéis associados</h4>
                        </div>
                        <div class="collapse show" id="utaListContent" >
                            <div class="card-body">
                                @php
                                    $userTypeAssignments = $user->userTypeAssignments()->orderBy('begin', 'ASC')->get();
                                @endphp
                                        ...
                                        <tbody>
                                            @foreach ($userTypeAssignments as $uta)
                                                <tr>
                                                    <td>
                                                        {{$uta->begin 
                                                            ? \Carbon\Carbon::parse($uta->begin)->isoFormat('DD/MM/Y') 
                                                            : '-'
                                                        }}
                                                    </td>
                                                    <td>
                                                        {{$uta->end 
                                                            ? \Carbon\Carbon::parse($uta->end)->isoFormat('DD/MM/Y') 
                                                            : '-'
                                                        }}    
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td class="text-center"><div class="d-inline-flex">
                                                        @can('userTypeAssignment-update')
                                                            <a href="{{ route('userTypeAssignments.edit', $uta) }}" data-bs-toggle="tooltip" title="Editar usuário" class="btn btn-primary btn-sm">
                                                                <i class="bi-pencil-fill"></i>
                                                            </a>&nbsp;          
                                                        @endcan
                                                        @can('userTypeAssignment-destroy')
                                                            <form name="{{ 'formDelete' . $uta->id }}" action="{{ route('userTypeAssignments.destroy', $uta) }}" method="POST">
                                                                @method('DELETE')
                                                                @csrf
                                                                <button type="button" data-bs-toggle="tooltip" title="Excluir usuário" 
                                                                    onclick="{{ 'if(confirm(\'Tem certeza que deseja excluir esse usuário?\')) document.forms[\'formDelete' . $uta->id . '\'].submit();' }}" class="btn btn-danger btn-sm">
                                ...
                                <div class="">
                                    <a class="btn btn-primary btn-sm mt-1" href="{{ route('userTypeAssignments.index', ['user_id' => $user->id]) }}" target="_blank">
                                        Listagem avançada...
                                    </a>    
                                </div>                       
                            </div>
